import type { Block, Transaction, Address, TransactionReceiptOptimism } from "../../../types";
import type {
  BaseBlock,
  BaseTransaction,
 ...
 */
export function transformBaseBlockToBlock(rpcBlock: BaseBlock): Block {
  ...
  return {
    ...
    transactions: Array.isArray(rpcBlock.transactions)
      ? rpcBlock.transactions.map((tx) =>
          typeof tx === "string" ? tx : (tx as BaseTransaction).hash,
        )
      : [],
    size: rpcBlock.size,
 ...
 */
export function transformBaseTransactionToTransaction(
  rpcTx: BaseTransaction,
  ...
  if (receipt) {
    const baseReceipt: TransactionReceiptOptimism = {
