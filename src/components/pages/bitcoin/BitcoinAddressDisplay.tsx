  ...
  ({ address, networkId }) => {
    ...
    const aiContext = useMemo(
      () => ({
        ...
        spendableBalanceBTC: address.balance,
        totalReceivedBTC: address.totalReceived,
        utxoCount: address.utxoCount,
        transactionCount: address.txCount,
        isGenesisAddress,
        recentTxids: address.txids?.slice(0, 10),
      }),
      [address, totalBalance, isGenesisAddress],
    );
    ...
    return (
      <div className="page-with-analysis">
            ...
            <div className="btc-tx-section">
              <div className="btc-empty-state">
                <p className="text-muted">
                  UTXO and transaction data requires wallet functionality or an indexing service.
                  The connected Bitcoin node may not have this data available.
                </p>
              </div>
            </div>
          )}
        ...
        <AIAnalysisPanel
          analysisType="bitcoin_address"
          context={aiContext}
          networkName={networkName}
          networkCurrency={networkCurrency}
          cacheKey={`openscan_ai_bitcoin_address_${networkId}_${address.address}`}
        />
      </div>
    );
  },
